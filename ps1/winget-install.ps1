...
<#=============================================================================
  ...
  用法:
    .\winget-install.ps1 <winget-export.json>
    .\winget-install.ps1 <PackageId1> [PackageId2 ...]
    .\winget-install.ps1 export.json PackageId1 PackageId2
...
function Invoke-WingetInstallWithProxy {
    ...
    for ($attempt = 1; $attempt -le ($MAX_RETRIES + 1); $attempt++) {
        ...
        if ($attempt -eq 1) {
            Write-Step ">>> [${attempt}] 启动 winget install --id ${PackageId} ..."
        } else {
            Write-Step ">>> [${attempt}] 重新安装 --id ${PackageId}（代理下载后重试）..."
        }

        $psi = [System.Diagnostics.ProcessStartInfo]@{
            FileName                = 'winget'
            Arguments               = "install --accept-package-agreements --accept-source-agreements --id `"$PackageId`""
            UseShellExecute         = $false
            RedirectStandardOutput  = $true
            RedirectStandardError   = $true
            CreateNoWindow          = $true
        ...
        if ($githubUrl -and $attempt -le $MAX_RETRIES) {
            ...
            try {
                ...
                Write-Success "  [v] 代理下载完成，准备重试 winget install"
            } catch {
        ...
        if ($proc.ExitCode -eq 0) {
            Write-Success "[OK] 安装成功: $PackageId"
        } else {
            Write-Fail "[FAIL] 安装失败: $PackageId，退出码: $($proc.ExitCode)"
        }
        break
    }
}
...
function Show-Help {
    ...
    Write-Host ""
    Write-Step "【用法】"
    Write-Step "  .\winget-install.ps1 <winget-export.json>"
    Write-Step "  .\winget-install.ps1 <PackageId1> [PackageId2 ...]"
    Write-Step "  .\winget-install.ps1 export.json PackageId1 PackageId2"
    Write-Host ""
    Write-Step "【说明】"
    Write-Step "  读取 winget export 导出的 JSON 文件或直接指定包 ID，批量执行"
    Write-Step "  winget install。安装过程中自动检测 GitHub 下载链接，中断后用"
    ...
    Write-Host ""
    Write-Step "【行为说明】"
    Write-Step "  1. 安装前通过 winget list 检查包是否已存在，已安装则跳过"
    Write-Step "  2. 安装时实时监控 winget 输出，检测 GitHub 下载链接"
    Write-Step "  3. 发现 GitHub 链接立即中断 winget，通过 gh-proxy 代理下载"
    Write-Step "  4. 下载完成后重试 winget install（最多重试 ${MAX_RETRIES} 次）"
    Write-Step "  5. 自动接受包许可协议和安全源协议（--accept-package-agreements）"
    Write-Host ""
    Write-Step "【配置】"
...
foreach ($pkg in $packages) {
    ...
    Write-Step "[>] 开始安装: ${pkg}"
    Write-Host ""
    Invoke-WingetInstallWithProxy $pkg
    Write-Host ""
}
